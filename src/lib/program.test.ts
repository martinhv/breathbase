...
describe("PROGRAM (Foundations)", () => {
  it("has exactly PROGRAM_LENGTH days", () => {
    expect(PROGRAM.days.length).toBe(PROGRAM_LENGTH);
  });
  it("days are numbered 1..PROGRAM_LENGTH in order", () => {
    PROGRAM.days.forEach((d, i) => expect(d.day).toBe(i + 1));
  });
  ...
  it("every day's duration is within the technique's allowed range", () => {
    for (const d of PROGRAM.days) {
...
describe("isProgramComplete", () => {
  it("is false when any day is missing", () => {
    ...
    expect(
      isProgramComplete({ ...enrolledFresh(), completedDays: all }),
    ).toBe(false);
  });
