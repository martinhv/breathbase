// Seven-day guided program. A curated tour of the foundational techniques,
// one per day, in an order that introduces complexity gradually:
//   slow breath → resonance → symmetry with holds → long exhale → focus.
//
...
export type ProgramState = {
  ...
  /** 1-indexed day numbers the user has finished; sorted ascending, unique. */
...
export const PROGRAM_LENGTH = PROGRAM.length;

export const getProgramDay = (day: number): ProgramDay | undefined =>
  PROGRAM.find((d) => d.day === day);
...
export const isProgramComplete = (state: ProgramState): boolean =>
  PROGRAM.every((d) => state.completedDays.includes(d.day));
